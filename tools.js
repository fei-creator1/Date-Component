...
export function getEnvUrl(that) {
    let baseUrl = ''
    process.env.NODE_ENV === 'production'
        ? (baseUrl = `${that.$config.apiUrl.pro}`)
        : (baseUrl = `${that.$config.apiUrl.dev}`)
    return baseUrl
}

/**
 * @description: js获取当前指定的前几天的日期
 * @param {*} n
...
// 时间初始化 2222
// today为布尔值，是否是从今天00：00：00开始，不是的话可不传
export function initDate(params) {
    // time, today, type
    const date = new Date(params.time)
    const year = date.getFullYear()
    const month = supplementZero(date.getMonth() + 1)
    const day = supplementZero(date.getDate())
    const hour = supplementZero(date.getHours())
    const min = supplementZero(date.getMinutes())
    const sec = supplementZero(date.getSeconds())
    ...
    const dateTime = dateType.get(params.type) || dateType.get('date1')
    if (params.today) {
        const startTime = year + '-' + month + '-' + day + ' ' + '00:00:00'
        return { startTime, nowTime: dateTime }
    } else {
        return dateTime
    }
}

function supplementZero(s) {
    return s >= 10 ? s : '0' + s
}
/**
 * @description: 获取日期或时间字符串
 * @author: huangyuhang
 * @param {String} type date-日期 datetime-日期和时间
 * @Date: 2023-01-11 09:20:03
 */
export const getDateStr = (timestamp, type = "date") => {
